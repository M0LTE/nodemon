...
public class TaitManager(IOptions<NodeMonConfig> config, ILogger<TaitManager> logger, IHubContext<NodeHub> hubContext, TaitSingleton taitSingleton) : IHostedService
{
    ...
    private static Stopwatch lastRssiReportedToUI = Stopwatch.StartNew();
    ...
    private Task Run(NodeMonConfig.Port port)
    {
        ...
        try
        {
            ...
            {
                if (lastRssiReportedToUI.Elapsed > TimeSpan.FromSeconds(0.1))
                {
                    lastRssiReportedToUI.Restart();
            ...
            {
                hubContext.Clients.All.SendAsync("VswrUpdate", new { v = Math.Round(args.Vswr, 2), p = port.Id });
